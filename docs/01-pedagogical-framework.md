...
> **Note on definitions.** The C1 / C2 / C3 *labels* and the *typical aim
> verbs* in the table above are sourced directly from the official Teaching
> Map (which uses these labels at the head of every unit aim). The
> short *definitions* in the third column are pedagogical paraphrases —
> the source uses the labels but does not provide formal definitions.
> Treat the definition column as `pending verification from official
> teaching map` if a finer-grained Ministry definition becomes available.

---

## 1.b. Stream Integrity Rule (mandatory)

Each stream is treated as **an independent pedagogical pathway**, not a
reduced or derived version of another stream. Concretely:

1. No stream's annual plan, scope-and-sequence, assessments, rubrics, or
   lesson plans may be generated by trimming, mapping, or transforming
   another stream's artefacts.
2. Each stream is authored from the official source documents on its own
   merit, even where the unit list overlaps with another stream's.
3. Cross-stream comparison is permitted for *teacher-facing
   documentation* (e.g. "this stream covers 4 of the same units as
   stream X") but is forbidden as a *generation mechanism*.
4. No "canonical" stream exists. Where this curriculum previously used
   the word "canonical" it means "first stream authored", not "stream
   from which others are derived".
5. Forced normalisation between streams (e.g. equalising unit count,
   re-ordering to match another stream, re-using another stream's
   project rubric verbatim) is forbidden.

This rule supersedes any earlier text suggesting derivation. Streams
that share a unit (e.g. Letters and Foreign Languages both teach
*Make Peace!*) get separately authored unit files at their respective
weekly hour load and depth.
